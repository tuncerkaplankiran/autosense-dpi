...
#include "../nDPIAPIIncludes/ndpi_main.h"
...
#include "../nDPIAPIIncludes/IfMcFePlugin_DecTypes.h"
#include "../nDPIAPIIncludes/linux_compat.h"
#include "../nDPIAPIIncludes/ndpi_protocols.h"
#include "../nDPIAPIIncludes/ndpi_typedefs.h"
#include "../ThirdParty/include/actypes.h"
								...
								ndpi_debug_function_ptr ndpi_debug_printf)
{
  ...
  ndpi_str = (ndpi_detection_module_struct *) ndpi_malloc(sizeof(struct ndpi_detection_module_struct));
...
/*
 *
 * This is the function that find port based protocol guess
 * Default protos set at the initDetectionModule function at startup
 * of this module, both source and destination address of the packet
 * processed if one not give result.
 *
 * Heap based search and comparison, port numbers are held on heap tree
 * cost: for N node logN int_32 comparison -> O(logN)
 * Recommendation: Port numbers can be held on hash table structure
 * gives us O(1):hash function processed and get exact index in hash array
 *
 *
 * returns short int type proto ID
 */
//----------------------------tuncer------------------------------------------
  ...
  */
  if(NDPI_COMPARE_PROTOCOL_TO_BITMASK(*detection_bitmask, ndpi_protocol_id) != 0) {
    ...
    */
    //-----------------tuncer-----------------------------------
    /*
     *Idx and function is like PPPI Plugin Interface members, func is the function
     *to be called after the protocol is detected
     *
     */
    //-----------------tuncer-----------------------------------
    ...
    //-------------tuncer---------------------------------------
    /*
     *protoIdx is actually is a match object,references callback_buffer
     *callback_buffer is actually a PPPI Parser Plugin Interface
     */
    //-------------------tuncer---------------------------------
...
#ifndef NDPI_PROTOCOL_BITTORRENT
			      ...
			      struct ndpi_flow_struct *flow)
{
	//------------------tuncer----------------------
	/*
	 * By now, flow (in ndpi context) flow->packet members are set already,
	 *
	 */
	//------------------tuncer----------------------
  ...
  if(tcph != NULL) {
    ...
    if((flow->next_tcp_seq_nr[0] == 0 && flow->next_tcp_seq_nr[1] == 0)
       ...
       */
    	...
    	//---------------tuncer---------------------------
    	/*
    	 * creation of flow parties (session party/client+server)
    	 * with initialization of their "NEXT" Sequence numbers
    	 *
    	 * In here, tcp->seq refers to SEND_UNA and next_tcp_seq_nr[]
    	 * refers to SEND_NEXT for send buffer
    	 */
    	//---------------tuncer---------------------------
...
void check_ndpi_other_flow_func(struct ndpi_detection_module_struct *ndpi_struct,  
  ...
  void (*func)(ndpi_detection_module_struct*, ndpi_flow_struct*);
  //void *func = NULL;
  u_int32_t a;
...
void check_ndpi_udp_flow_func(struct ndpi_detection_module_struct *ndpi_struct,  
  ...
  if(flow->packet.payload_packet_len>0)
   {
  IDL_L7_PROTOCOL_NAME name = IDL_L7_PNAME_ANY;;
  IDL_L7_PROTOCOL_ROLE role1 = IDL_L7_PROLE_ANY;
  IDL_L7_PROTOCOL_ROLE role2 = IDL_L7_PROLE_ANY;

if(flow->asd.m_eStatus!=AS_STATE_SUCCESS && flow->asd.m_eStatus!=AS_STATE_FAIL)
{
 flow->asd.m_pRagel2->SenseProtocol(flow,flow->asd,(size_t)flow->packet.packet_direction,name,role1,role2);

 if (name != IDL_L7_PNAME_ANY)
 {
 flow -> m_eAutoSenseDetected_proto = name;
 /*
 cout<<"**************AUTOSENSEEEEE*********"<<endl;
 cout<<"PROTOCOL WITH ID FOR UDP: "<<name<<" FOUND "<<"with source "<<flow->packet.iph->saddr <<endl;
 flow->asd.m_eStatus = AS_STATE_SUCCESS;// Sensor is finished
 cout<<"**************AUTOSENSEEEEE*********"<<endl;
 */
 }
 ...
 {
 //cout<<"*********AUTOSENSEEEEE NOT FOUND ANY PROTOCOL FOR UDP*****"<<endl;
 flow->asd.m_eStatus = AS_STATE_FAIL; // Sensor is finished
 }
 ...
 {
 AutoSenseStateController *pfsm = &(flow->asd.m_machines.fsm[party][i]);
 pfsm->finished = true;
 }
 }
...
if(flow -> m_eAutoSenseDetected_proto != IDL_L7_PNAME_ANY)
{
	 cout<<"**************AUTOSENSEEEEE*********"<<endl;
	 cout<<"PROTOCOL WITH ID FOR UDP: "<<name<<" FOUND "<<"with source "<<flow->packet.iph->saddr <<endl;
	 flow->asd.m_eStatus = AS_STATE_SUCCESS;// Sensor is finished
...
void check_ndpi_tcp_flow_func(struct ndpi_detection_module_struct *ndpi_struct,  
  ...
  if(flow->packet.payload_packet_len != 0) {
    ...
    //----------------------------tuncer-----------------------------------
    /*
     * Add AutoSense Implementation here
     */
    //----------------------------tuncer-----------------------------------
    ...
    if(flow->asd.m_eStatus!=AS_STATE_SUCCESS && flow->asd.m_eStatus!=AS_STATE_FAIL)
    {
    	bool fNeedMore = flow->asd.m_pRagel2->SenseProtocol(flow,flow->asd,(size_t)flow->packet.packet_direction,name,role1,role2);

		if (name != IDL_L7_PNAME_ANY)
		{
			cout<<"**************AUTOSENSEEEEE*********"<<endl;
			cout<<"PROTOCOL WITH ID: "<<name<<" FOUND "<<endl;
			flow->asd.m_eStatus = AS_STATE_SUCCESS;// Sensor is finished
			cout<<"**************AUTOSENSEEEEE*********"<<endl;
		}
		else if (fNeedMore)
		...
		{
			cout<<"*********AUTOSENSEEEEE NOT FOUND ANY PROTOCOL*****"<<endl;
			flow->asd.m_eStatus = AS_STATE_FAIL; // Sensor is finished
		}
    ...
    //----------------------------tuncer------------------------------------
    /*
     *If the above if block does not give any detected protocol, then this
     *if block executed. In this if block, all the protocols specified as
     *tcp protocol parser codes (*fn) called (http.cpp,mail-pop.cpp etc), when payload
     *data (string) matches one of these parsers break from loop and return.
     *
     *In short: this if block of the code is for protocol not coming from
     *their own port, i.e port=80 but protocol is not HTTP etc. This is kind of
     *autosense but very very very inefficient. Because each protocol parser code
     *is called, say n protocol and payload data string length is n, then O(n^2)
     *
     *However in our ragel autosense implementation, n length data string and
     *it is not the string comparison, uint comparison used, but let say it O(n).
     */
    //----------------------------tuncer------------------------------------
					   ...
					   struct ndpi_id_struct *dst)
{
  ...
  if (!(flow->asd.m_pRagel2))
  {
	  flow->asd.m_pRagel2 = new CTrafficAutosenseManager_Ragel2;
  }
  ...
  //-----------------tuncer-----------------------------
  /*
   * ndpi_connection_tracking function is for setting metadata
   * (sequence numbers of flow parties etc..) about flow
   * according to this metadata ndpi_selection_packet bitmask set.
   */
  //-----------------tuncer----------------------
...
//--------------------tuncer-----------------------------
/*
 * This function caches the real and correlated protocol
 * info about the current flow(session) as a stack implementation
 *
 *flow->protocol_stack_info.entry_is_real_protocol refers to the stack index of
 *the inserted protocol, if protocol to be inserted is a real protocol (HTTP,FTP,POP etc..)
 *it is inserted at the position of the top-most real protocol or
 *below the last non-unknown correlated protocol(Twitter,FaceBook,Gmail etc..).
 *
 *if the new protocol is not real, put it on top of stack but if there is
 *a real protocol in the stack, make sure at least one real protocol remains in the stack
 *
 *In short:
 *for flow->detected_protocol_stack[]:
 *stack[0] refers to the latest detected correlated protocol over real protocol
 *which is the latest detected real protocol located on stack[usually at the last index]
 *
 * One real protocol and one correlated protocol over real protocol on stack at a time
 *
 */
//--------------------tuncer-----------------------------
