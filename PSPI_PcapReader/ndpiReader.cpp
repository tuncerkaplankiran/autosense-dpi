...
//------------------tuncer------------------------------
	  /*
	   * ip-header should be at least 5 word(32bit) long = 5*4 = 20byte
	   */
...
//------------------tuncer------------------------------
  ...
  *proto = iph->protocol;
  ...
  //------------------tuncer--------------------------------------
  /*
   * protocol number "6" in ip header refers to TCP, "17" refers to UDP
   * Can be seen in wireshark
   * For Further see NimsDecoder
   */
  //------------------tuncer--------------------------------------
  ...
  if(iph->protocol == 6 && l4_packet_len >= 20) {
    ...
    //------------tuncer -----------------------------
    /*
     * smaller addr (src,dest) in ip header leads to lower port
     * and lower address,i.e lower_ip = lower_addr,lower_port = lower_addr->port
     * port info comes from tcp header
     *
     */
    //------------tuncer -----------------------------
  ...
  //-------------------tuncer---------------------------------------------
  /*
   * 5 tuple in here source ip+port,destination ip+port,protocol
   * this is common to all "session tracking" network systems
   * idx is the hash function key here
   */
  //-------------------tuncer---------------------------------------------
  ...
  if(ret == NULL) {
	...
	memset(newflow->dst_id, 0, size_id_struct);
      ...
      *src = (ndpi_id_struct*)newflow->src_id, *dst = (ndpi_id_struct*)newflow->dst_id;
    ...
    if(flow->lower_ip == lower_ip && flow->upper_ip == upper_ip
       && flow->lower_port == lower_port && flow->upper_port == upper_port)
      *src = (ndpi_id_struct*)flow->src_id, *dst = (ndpi_id_struct*)flow->dst_id;
    else
      *src = (ndpi_id_struct*)flow->dst_id, *dst = (ndpi_id_struct*)flow->src_id;
...
static unsigned int packet_processing(u_int16_t thread_id,
  ...
  //----------------tuncer ------------------------------
  /*
   * ndpi_flow *flow is like a "session" object in the Nims Code Base
   * ndpi_thread_info[thread_id].stats is like MP(Measurement Processor)
   */
  //----------------tuncer ------------------------------
